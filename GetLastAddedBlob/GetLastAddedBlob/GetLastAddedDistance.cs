    ...
    public static class GetLastAddedDistance
    {
            ...
            ILogger log)
        {
            ...
            try
            {
                ...
                using HttpResponseMessage idResponse = await httpClient.GetAsync($"https://shamboo-backend.azure-api.net/tableService/deviceId?email={userEmail[0]}&password={userPassword[0]}");
                idResponse.EnsureSuccessStatusCode();
                string fetchedDeviceID = await idResponse.Content.ReadAsStringAsync();

                if (fetchedDeviceID != deviceID[0])
                {
                    return new BadRequestObjectResult(-1);
                }
                ...
                using HttpResponseMessage distancesResponse = await httpClient.GetAsync($"https://databaseshamboo.table.core.windows.net/measurements{MEASUREMENTS_SAS}&$filter=device_id%20eq%20'{deviceID[0]}'&$select=distance,Timestamp");
